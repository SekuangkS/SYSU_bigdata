...
class ResumeRecommendationTrainer:
    ...
    def save_model_and_artifacts(self):
        ...
        try:
            vectorizer_params = {
                'vocabulary_': {k: int(v) for k, v in self.skill_vectorizer.vocabulary_.items()} 
                    if hasattr(self.skill_vectorizer, 'vocabulary_') else {},
                'idf_': [float(x) for x in self.skill_vectorizer.idf_] 
                    if hasattr(self.skill_vectorizer, 'idf_') else [],
                'max_features': int(getattr(self.skill_vectorizer, 'max_features', 1000)),
            }

            scaler_params = {
                'mean_': [float(x) for x in self.scaler.mean_] if hasattr(self.scaler, 'mean_') else [],
                'scale_': [float(x) for x in self.scaler.scale_] if hasattr(self.scaler, 'scale_') else [],
                'var_': [float(x) for x in self.scaler.var_] if hasattr(self.scaler, 'var_') else [],
                'n_features_in_': int(getattr(self.scaler, 'n_features_in_', 0)),
                'n_samples_seen_': int(getattr(self.scaler, 'n_samples_seen_', 0)),
            }

            preprocessor_params = {
                'vectorizer_params': vectorizer_params,
                'scaler_params': scaler_params,
                'feature_dim': resume_feature_dim  # 使用模型的特征维度
            }
            ...
            # 使用临时文件确保原子性写入
            temp_path = 'models/preprocessor_params.json.tmp'
            with open(temp_path, 'w', encoding='utf-8') as f:
                json.dump(preprocessor_params, f, indent=2, ensure_ascii=False)
            
            # 重命名为正式文件
            if os.path.exists(temp_path):
                if os.path.exists('models/preprocessor_params.json'):
                    os.remove('models/preprocessor_params.json')
                os.rename(temp_path, 'models/preprocessor_params.json')

        except Exception as e:
            print(f"保存JSON预处理器参数失败: {e}")
            traceback.print_exc()
    ...
    def train_coarse_ranker(self, resume_features, job_features, labels):
        """训练粗排模型"""
        ...
        # 准备粗排特征
        X = np.concatenate([resume_features, job_features], axis=1)
        y = labels
        ...
        # 保存模型
        os.makedirs('models', exist_ok=True)
        joblib.dump(model, 'models/coarse_ranker.pkl')
        
        # 评估
        train_score = model.score(X_train, y_train)
        test_score = model.score(X_test, y_test)
        print(f"粗排模型训练完成 - 训练集准确率: {train_score:.4f}, 测试集准确率: {test_score:.4f}")
        return model
    ...
    def run_training_pipeline(self):
        ...
        try:
            ...
            # 训练粗排模型
            self.train_coarse_ranker(resume_features, job_features, labels)
